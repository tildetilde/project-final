...
mongoose
  ...
  .catch((err) => {
    ...
    console.log(
      "Server is running without database connection for development..."
    );
...
const handleSpotifyError = (err: unknown, res: Response, operation: string) => {
  ...
  if (err && typeof err === "object" && "statusCode" in err) {
    ...
    if (statusCode === 403) {
      res
        .status(403)
        .json({ error: "Premium account required for playback control." });
...
app.put("/spotify/play", async (req: Request, res: Response) => {
  try {
    const { trackId, contextUri, uris, deviceId, positionMs, offset } =
      req.body;
    const accessToken = req.session.accessToken;
...
app.put("/spotify/pause", async (req: Request, res: Response) => {
  try {
    ...
    const pauseOptions: { device_id?: string } = {};
    if (deviceId) {
...
app.post("/spotify/next", async (req: Request, res: Response) => {
  try {
    const { deviceId } = req.body;
    const accessToken = req.session.accessToken;
    ...
    const nextOptions: { device_id?: string } = {};
    if (deviceId) {
...
app.post("/spotify/previous", async (req: Request, res: Response) => {
  try {
    const { deviceId } = req.body;
    const accessToken = req.session.accessToken;
    ...
    const previousOptions: { device_id?: string } = {};
    if (deviceId) {
...
app.get("/spotify/devices", async (req: Request, res: Response) => {
  try {
    const accessToken = req.session.accessToken;
...
app.get("/spotify/playback-state", async (req: Request, res: Response) => {
  try {
    const accessToken = req.session.accessToken;
...
app.put("/spotify/transfer", async (req: Request, res: Response) => {
  try {
...
app.get("/quiz-questions/random", async (req: Request, res: Response) => {
  try {
    ...
    if (!randomQuestion) {
      return res
        .status(404)
        .json({ error: "Could not find a random question" });
    }
...
async function populateQuizQuestionsFromPlaylist() {
  ...
  try {
    ...
    if (quizQuestions.length > 0) {
      ...
      console.log(
        `Database has been populated with ${quizQuestions.length} questions from the playlist.`
      );
    } else {
